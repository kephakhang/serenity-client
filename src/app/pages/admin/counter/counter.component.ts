  ...
  constructor(public auth: AuthServiceProvider, private service: SmartTableData) {
    this.auth.getSession().then(user => {
      this.auth.get('/api/v1/tenant/list').then((list: any[]) => {
        this.tenantList = list.filter(item => item.type === 1 && item.hostname).map(item => new TenantData(item.id, item.name, item.type, item.description, item.countryId, item.hostUrl, item.prefix, item.hostname, new Date(item.regDatetime).toLocaleString(), new Date(item.modDatetime).toLocaleString()))
